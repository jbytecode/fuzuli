...
Expression *AstBuilder::fuzuliCall(Token *tok, vector<Expression*> expr) {
	if (tok->getType() == PLUS) {
		...
		return (pe);
			...
			&& strcmp(tok->getContent(), "first") == 0) {
		FirstExpression *pe = new FirstExpression(expr);
		return (pe);
	}else if (tok->getType() == IDENTIFIER
			&& strcmp(tok->getContent(), "last") == 0) {
