...
Token *FunctionExpression::eval(Environment *env){
	...
	str_func_name << ((IdentifierExpression*)this->expressions[0])->stringToken->getContent();
	str_func_name << this->expressions[1]->expressions.size();

	func->name = new StringExpression(new Token(str_func_name.str().c_str(), STRING));
...
Token *FunctionCallExpression::eval(Environment *env){
	...
	if(func==NULL){
		cout << "Fuzuli Function "<<fname->getContent() << " is not defined."<<endl;
		exit(-1);
	}
